...
const HOME_SEO = {
  title: "World Clock, Weather & Local Time in 500+ Cities | TimeInCity",
  description:
...
const CANONICAL_BASE_URL = "https://www.timeincity.com";

function updateHeadMetadata(meta: { title: string; description: string; path: string }): void {
  if (typeof document === "undefined") {
    return;
  }
  const normalizedPath = meta.path.startsWith("/") ? meta.path : `/${meta.path}`;
  const absolutePath = normalizedPath === "/" ? "/" : normalizedPath.replace(/\/+$/, "");
  const absoluteUrl = `${CANONICAL_BASE_URL}${absolutePath === "/" ? "/" : absolutePath}`;
...
export default function App(): JSX.Element {
  ...
  const [regionPage, setRegionPage] = useState<RegionPageSlug | null>(() => {
    if (typeof window === "undefined") {
      return null;
    }
    return getRegionByPath(window.location.pathname);
  });
  ...
  const regionContent = regionPage ? REGION_PAGE_CONTENT[regionPage] : undefined;
  ...
  useEffect(() => {
    ...
    const handler = () => {
      ...
      const staticMatch = getStaticPageFromPath(path);
      if (staticMatch) {
        setStaticPage(staticMatch);
        setRegionPage(null);
        return;
      }

      const regionMatch = getRegionByPath(path);
      if (regionMatch) {
        setRegionPage(regionMatch);
        setStaticPage(null);
      } else {
        setRegionPage(null);
        setStaticPage(null);
  ...
  const regionCities = useMemo(() => {
    if (!regionPage || !regionContent) {
      return [] as CityConfig[];
    }
    return getRegionCities(regionPage, CITY_CONFIG_LIST);
  }, [regionPage, regionContent]);
  ...
  useEffect(() => {
    if (staticPage && staticContent) {
      updateHeadMetadata({
        ...
        path: getStaticPagePath(staticPage)
      });
      return;
    }
    ...
    if (regionPage && regionContent) {
      updateHeadMetadata({
        title: regionContent.metaTitle,
        description: regionContent.metaDescription,
        path: regionContent.path
      });
      return;
    }
    ...
    if (isCityRoute && activeCityConfig) {
      ...
      updateHeadMetadata({
        ...
        path: `/city/${activeCityConfig.slug}`
      });
      return;
    }

    updateHeadMetadata({ ...HOME_SEO, path: "/" });
  }, [staticPage, staticContent, regionPage, regionContent, isCityRoute, activeCityConfig]);
  ...
  const handleNavigateToRegionPage = (slug: RegionPageSlug) => {
    if (typeof window !== "undefined") {
  ...
  if ((staticPage && staticContent) || (regionPage && regionContent)) {
    const isRegionView = Boolean(regionPage && regionContent);
    const detailSubtitle = isRegionView
      ? `Browse curated clocks for ${regionContent!.label}.`
      : `Learn more about ${staticContent!.title}.`;
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-slate-100 via-white to-slate-100 text-slate-900 transition-colors dark:from-slate-950 dark:via-slate-950 dark:to-slate-900 dark:text-slate-100">
        ...
        <SiteHeader
          ...
          onNavigateRegion={handleNavigateToRegionPage}
        ...
        <main className="mx-auto flex w-full max-w-4xl flex-col gap-8 px-4 pb-10 pt-6 sm:px-6 lg:px-8">
          <header className="flex flex-wrap items-center justify-between gap-4 rounded-3xl border border-slate-200/70 bg-white/80 px-6 py-4 shadow-lg shadow-slate-900/5 backdrop-blur dark:border-slate-800 dark:bg-slate-900/70">
            <div>
              ...
              <p className="text-sm text-slate-600 dark:text-slate-400">{detailSubtitle}</p>
            </div>
            <button
              type="button"
          ...
          {isRegionView ? (
            <RegionPage content={regionContent!} cities={regionCities} onSelectCity={handleCityConfigSelect} />
          ) : (
            <>
        ...
        <SiteFooter onNavigate={handleNavigateToStaticPage} currentPage={isRegionView ? null : staticPage} />
