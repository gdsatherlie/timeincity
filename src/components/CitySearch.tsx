...
export function CitySearch({ onSelectCity, placeholder = "Search city…", label }: CitySearchProps): JSX.Element {
  ...
  const handleSelect = (city: NormalizedCity) => {
    ...
    setQuery(formatCityDisplay({ name: city.name, region: city.state ?? undefined, country: city.country }));
    setIsFocused(false);
  };
  ...
  return (
    <div className="relative">
      ...
      />
      {visible ? (
        <ul className="absolute left-0 right-0 z-30 mt-2 max-h-80 overflow-y-auto rounded-2xl border border-slate-200 bg-white/95 p-2 shadow-2xl shadow-slate-900/20 dark:border-slate-800 dark:bg-slate-900">
          {results.map((city, index) => (
            <li key={city.slug}>
              ...
              >
                <span className="font-semibold">{formatCityDisplay({ name: city.name, region: city.state ?? undefined, country: city.country })}</span>
