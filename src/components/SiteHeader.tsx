...
import type { RegionPageSlug } from "../data/regions";
import { REGION_PAGE_CONTENT } from "../data/regions";
...
interface SiteHeaderProps {
  ...
  onNavigateRegion: (slug: RegionPageSlug) => void;
...
type MenuItem =
  ...
  | { type: "region"; label: string; slug: RegionPageSlug };

const REGION_ORDER: RegionPageSlug[] = [
  "all-cities",
  "united-states",
  "europe",
  "asia",
...
const CITY_MENU: MenuItem[] = REGION_ORDER.map((slug) => ({
  type: "region",
  slug,
  label:
    slug === "all-cities"
      ? `All Cities (${POPULAR_CITIES_COUNT.toLocaleString()})`
      : REGION_PAGE_CONTENT[slug].label
}));
