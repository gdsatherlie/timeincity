    ...
    <section className="flex flex-col gap-4 rounded-3xl border border-slate-200/70 bg-white/70 p-6 shadow-lg shadow-slate-900/5 backdrop-blur dark:border-slate-800 dark:bg-slate-900/70">
      <header className="flex flex-col gap-1">
        ...
        <p className="text-sm text-slate-600 dark:text-slate-400">
          Browse {CITY_COUNT.toLocaleString()} major destinations. Scroll the list and tap a city name to jump straight to its time zone.
        </p>
      </header>
      <div className="max-h-[32rem] overflow-y-auto pr-1">
        <div className="grid grid-cols-2 gap-y-2 gap-x-4 sm:grid-cols-3 lg:grid-cols-4">
          {CITY_LIST.map((city) => {
